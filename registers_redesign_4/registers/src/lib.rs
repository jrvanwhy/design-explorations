//! # Addresses and Buses
//!
//! There are many different types of registers, including:
//! 1. MMIO registers
...
//! have a generic argument for their address type.
//!
//! For most registers, the address type can be dynamically offset (i.e. you can perform addition
//! on it) without changing its type. Those address types implement the [DynAddress] trait. RISC-V
//! CSRs, however, require their addresses to be `const`, so they do not implement DynAddress.
//!
//! In addition, address types should implement [`Bus<T>`] for every type `T` that they support.
